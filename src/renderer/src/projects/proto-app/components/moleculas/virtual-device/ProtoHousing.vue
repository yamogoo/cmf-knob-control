...
import { type KnobEmittedData } from '@typings/index'
...
const onKnobTurnLeft = (data: KnobEmittedData): void => {
  document.dispatchEvent(
    createCustomEvent('knobturnleft', {
      detail: data,
      bubbles: true
    })
  )
...
const onKnobTurnRight = (data: KnobEmittedData): void => {
  document.dispatchEvent(
    createCustomEvent('knobturnright', {
      detail: data,
      bubbles: true
    })
  )
